-- скрапер TVS для загрузки плейлиста "CLİPTV" https://cliptv.az (6/1/21)
...
-- ##
	...
	local function LoadFromSite()
		...
		local url = 'http://fe.cliptv.az'
		local headers = 'Content-Type: application/json;charset=UTF-8'
		local rc, answer = m_simpleTV.Http.Request(session, {url = url .. '/api/v1/device/authorize?uuid=' .. decode64('RTQ6Mjc6NzE6NEM6NjU6NUM'), headers = headers})
			if rc ~= 200 then
				m_simpleTV.Http.Close(session)
			 return
			end
		rc, answer = m_simpleTV.Http.Request(session, {url = url .. '/api/v1/channels/list?lang=en', headers = headers})
		m_simpleTV.Http.Close(session)
		...
		local t, i = {}, 1
			while tab.result.list[i] do
				...
				t[i].address = tab.result.list[i].url .. '$OPT:http-user-agent=Mozilla/5.0 (Linux; Android 10; SAMSUNG-SM-T377A Build/NMF26X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.105 Mobile Safari/537.36$OPT:NO-STIMESHIFT'
				i = i + 1
			end
